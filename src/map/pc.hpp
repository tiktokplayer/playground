...
// @packetfilter bit flags [Custom]
enum e_packet_filter : uint32 {
	PF_NONE          = 0x0000,
	PF_ITEM          = 0x0001, // Other players' item use animations
	PF_EMOTE         = 0x0002, // Emoticons
	PF_CHAT          = 0x0004, // Public chat of other players
	PF_BUFF_ANIM     = 0x0008, // Status/buff effects shown on other players
	PF_ATK_ANIM      = 0x0010, // Normal attacks between others (animation and damage numbers)
	PF_SKILL_TARGET  = 0x0020, // Targeted skill animations between others
	PF_SKILL_GROUND  = 0x0040, // Ground skill cast animations of others
	PF_SONG_DANCE    = 0x0080, // Ground effect of songs and dances of others
	PF_DIR_CHANGE    = 0x0100, // Direction changes of others
	PF_PARTY_INFO    = 0x0200, // (not used any more, kept so old values keep their meaning)
	PF_PET_EMOTION   = 0x0400, // Pet emotions
	PF_AURA          = 0x0800, // Effects (auras, spirit balls...) on other players
	PF_HAT_EFFECT    = 0x1000, // Hat effects
	PF_SKILL_UNITS   = 0x2000, // Ground effect of every area skill of others (the units the client draws: Storm Gust, Volcano...)
	PF_ALL           = 0x3DFF, // everything above except the unused PF_PARTY_INFO
	// "safe" preset: pure decoration, nothing that changes what you can read of a fight
	PF_SAFE          = PF_ITEM | PF_EMOTE | PF_BUFF_ANIM | PF_SONG_DANCE | PF_DIR_CHANGE | PF_PET_EMOTION | PF_AURA | PF_HAT_EFFECT,
};

// @hide summons (roulette menu) bit flags [Custom]
enum e_summon_hide : uint32 {
	SH_NONE        = 0x00,
	SH_PET_OTHERS  = 0x01, // pets of other players
	SH_PET_OWN     = 0x02, // the player's own pets
	SH_ELEMENTAL   = 0x04, // elementals of other players (Elemental Master)
	SH_HOMUNCULUS  = 0x08, // homunculi of other players
	SH_ABR         = 0x10, // ABR of other players (Meister)
	SH_BIONIC      = 0x20, // bionics of other players (Biolo)
	SH_WARG        = 0x40, // wargs of other players (Ranger; a flag of the player, not a unit)
	SH_ALL         = 0x7F,
};
...
public:
	...
	// @packetfilter flags (bitfield)
	uint32 packet_filter;
	// @battleinfo toggle
	bool battleinfo;
	uint16 last_dmg_skill; // Skill id of the last damaging hit, used by @battleinfo
	t_tick last_reflect_tick; // when magic aimed at someone was last bounced back onto this player (Kaite, Magic Mirror...), for @battleinfo
	uint16 last_reflect_skill; // ...and which skill it was
	// @hold toggle
	bool hold_position;
	// @spb timer
    int32 spb_tid;
	uint32 spb_hash; // what the party window was last sent with by @spb (0 = send again)
	uint32 summon_hide; // e_summon_hide: which summons this player does not want to see (kept in the character variable HIDE_SUM)
	uint32 spb_hide; // @spb: bit i set = buff i of the table is not shown for this player (kept in the character variable SPB_HIDE)
...
void pc_summonhide_set(map_session_data *sd, uint32 mask); // roulette menu: hide summons, kept in the character variable HIDE_SUM
void pc_packetfilter_set(map_session_data *sd, uint32 mask); // @packetfilter: set and keep in the character variable PF_MASK
