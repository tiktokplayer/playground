...
/*==========================================
 * @packetfilter - Filter visual packets to reduce WoE lag [Custom]
 *------------------------------------------*/
ACMD_FUNC(packetfilter)
{
	nullpo_retr(-1, sd);

	static const struct { const char *name; uint32 flag; const char *what; } opts[] = {
		{ "item",   PF_ITEM,         "other players using items" },
		{ "emote",  PF_EMOTE,        "emoticons" },
		{ "chat",   PF_CHAT,         "public chat of other players" },
		{ "buff",   PF_BUFF_ANIM,    "status effects shown on other players" },
		{ "atk",    PF_ATK_ANIM,     "attacks between others (animation and damage numbers)" },
		{ "skill",  PF_SKILL_TARGET, "targeted skill animations between others" },
		{ "ground", PF_SKILL_GROUND, "ground skill cast animations of others" },
		{ "song",   PF_SONG_DANCE,   "ground effect of songs and dances" },
		{ "dir",    PF_DIR_CHANGE,   "direction changes of others" },
		{ "pet",    PF_PET_EMOTION,  "pet emotions" },
		{ "aura",   PF_AURA,         "effects and auras on other players" },
		{ "hat",    PF_HAT_EFFECT,   "hat effects" },
		{ "units",  PF_SKILL_UNITS,  "the ground effect of area skills of others (hides AoE you cannot see anyway)" },
	};

	auto say_state = [&]() {
		char on[200] = "", off[200] = "", line[260];
		for (const auto &o : opts) {
			char *dst = (sd->packet_filter & o.flag) ? on : off;
			if (dst[0]) strcat(dst, " ");
			strcat(dst, o.name);
		}
		snprintf(line, sizeof(line), "[PacketFilter] Filtering: %s", on[0] ? on : "nothing");
		clif_displaymessage(fd, line);
		if (on[0]) {
			snprintf(line, sizeof(line), "[PacketFilter] Not filtered: %s", off[0] ? off : "nothing");
			clif_displaymessage(fd, line);
		}
	};

	if (!message || !*message) {
		clif_displaymessage(fd, "=== @packetfilter: hide other players' visual effects to reduce lag ===");
		say_state();
		clif_displaymessage(fd, "Presets:  safe (decoration only)  |  woe  |  all  |  off");
		clif_displaymessage(fd, "Options (+ turns one on, - turns it off), for example:  @packetfilter +emote +buff -chat");
		for (const auto &o : opts) {
			char line[200];
			snprintf(line, sizeof(line), "  %-7s %s", o.name, o.what);
			clif_displaymessage(fd, line);
		}
		clif_displaymessage(fd, "Your own actions, hits on you, skills aimed at you and your own skill effects are never filtered.");
		return 0;
	}
	...
	if (strcmpi(msg, "off") == 0) {
		pc_packetfilter_set(sd, PF_NONE);
		clif_displaymessage(fd, "[PacketFilter] All filters disabled.");
		return 0;
	}
	if (strcmpi(msg, "all") == 0) {
		pc_packetfilter_set(sd, PF_ALL);
		clif_displaymessage(fd, "[PacketFilter] Filtering everything visual (including public chat and area-skill effects).");
		return 0;
	}
	if (strcmpi(msg, "safe") == 0) {
		pc_packetfilter_set(sd, PF_SAFE);
		say_state();
		return 0;
	}
	if (strcmpi(msg, "woe") == 0) {
		// WoE preset: safe + public chat
		pc_packetfilter_set(sd, PF_SAFE | PF_CHAT);
		say_state();
		return 0;
	}

	// Parse +/- toggles
	for (char *token = strtok(msg, " "); token; token = strtok(nullptr, " ")) {
		bool add = true;
		const char *name = token;
		if (name[0] == '+') { add = true; name++; }
		else if (name[0] == '-') { add = false; name++; }

		uint32 flag = PF_NONE;
		for (const auto &o : opts)
			if (strcmpi(name, o.name) == 0) { flag = o.flag; break; }

		if (!flag) {
			char errmsg[128];
			snprintf(errmsg, sizeof(errmsg), "[PacketFilter] Unknown option: %s (@packetfilter shows them all)", name);
			clif_displaymessage(fd, errmsg);
			continue;
		}
		pc_packetfilter_set(sd, add ? (sd->packet_filter | flag) : (sd->packet_filter & ~flag));
	}
...
/*==========================================
 * @battleinfo - Toggle kill/death notifications with skill info [Custom]
 *------------------------------------------*/
ACMD_FUNC(battleinfo)
{
	nullpo_retr(-1, sd);

	sd->battleinfo = !sd->battleinfo;

	if (sd->battleinfo)
		clif_displaymessage(fd, "[BattleInfo] Enabled. You will see kill/death notifications with skill names.");
	else
		clif_displaymessage(fd, "[BattleInfo] Disabled.");

	return 0;
}

/*==========================================
 * @battlestats - Show combat stats of the player [Custom]
 *------------------------------------------*/
ACMD_FUNC(battlestats)
{
	...
	auto say = [&](const char *fmt, auto... args) {
		char buf[CHAT_SIZE_MAX];
		safesnprintf(buf, sizeof(buf), fmt, args...);
		clif_displaymessage(fd, buf);
	};

	// Sends a titled list with one entry per line, skipping zero values
	auto show_list = [&](const char *title, const char *const *names, const int32 *values, int32 count) {
		bool any = false;
		...
		for (int32 i = 0; i < count; i++) {
			...
			say("  %s: %+d%%", names[i], values[i]);
		}
	};

	static const char *const race_names[RC_MAX] = { "Formless", "Undead", "Brute", "Plant", "Insect", "Fish", "Demon", "Demihuman", "Angel", "Dragon", "Player", "Doram", "All" };
	static const char *const size_names[SZ_MAX] = { "Small", "Medium", "Large", "All" };
	static const char *const ele_names[ELE_MAX] = { "Neutral", "Water", "Earth", "Fire", "Wind", "Poison", "Holy", "Shadow", "Ghost", "Undead", "All" };

	say("===== Battle Stats: %s =====", sd->status.name);

	// Offense
	say("-- Offense --");
	say("  ATK: %d + %d", status->batk, status->rhw.atk + status->rhw.atk2);
	say("  MATK: %d ~ %d", status->matk_min, status->matk_max);
	say("  P.ATK: %d", status->patk);
	say("  S.MATK: %d", status->smatk);
	say("  ASPD: %d", (AMOTION_ZERO_ASPD - status->amotion) / AMOTION_INTERVAL);
	say("  HIT: %d", status->hit);
	say("  CRIT: %.1f", status->cri / 10.0);
	say("  Crit Damage: %d%%", 140 + sd->bonus.crit_atk_rate);
	say("  H.Plus: %d", status->hplus);
	say("  C.Rate: %d", status->crate);
	say("  Melee Damage: %+d%%", sd->bonus.short_attack_atk_rate);
	say("  Ranged Damage: %+d%%", sd->bonus.long_attack_atk_rate);
	say("  Skill Delay: %+d%%", -sd->bonus.delayrate);

	// Defense
	say("-- Defense --");
	say("  DEF: %d + %d", status->def, status->def2);
	say("  MDEF: %d + %d", status->mdef, status->mdef2);
	say("  FLEE: %d", status->flee);
	say("  Perfect Dodge: %.1f", status->flee2 / 10.0);
	say("  RES: %d", status->res);
	say("  MRES: %d", status->mres);
	say("  Melee Reduction: %+d%%", sd->bonus.near_attack_def_rate);
	say("  Ranged Reduction: %+d%%", sd->bonus.long_attack_def_rate);

	// Cast time (Renewal): variable cast is reduced by DEX/INT, then by items/buffs
	{
		double var_pct = (1.0 - sqrt((double)(status->dex * 2 + status->int_) / battle_config.vcast_stat_scale)) * 100.0;
		double var_item = (double)sd->bonus.varcastrate;
		double fix_item = (double)-sd->bonus.fixcastrate;

		if (var_pct < 0)
			var_pct = 0;

		say("-- Cast Time --");
		say("  Variable Cast (stats): %.1f%%", var_pct);
		say("  Variable Cast (items): %+.0f%%", var_item);
		say("  Variable Cast (final): %.1f%%", var_pct * (1.0 - min(var_item, 100.0) / 100.0));
		say("  Fixed Cast Reduction: %.0f%%", min(fix_item, 100.0));
		say("  Extra Variable Cast: %+d ms", sd->bonus.add_varcast);
		say("  Extra Fixed Cast: %+d ms", sd->bonus.add_fixcast);
	}
	...
	for (int32 i = 0; i < ELE_MAX; i++)
		vals[i] = sd->indexed_bonus.subele[i];
	show_list("Resist vs Element", ele_names, vals, ELE_MAX);
	for (int32 i = 0; i < RC_MAX; i++)
		vals[i] = sd->indexed_bonus.subrace[i];
	show_list("Resist vs Race", race_names, vals, RC_MAX);
	for (int32 i = 0; i < SZ_MAX; i++)
		vals[i] = sd->indexed_bonus.subsize[i];
	show_list("Resist vs Size", size_names, vals, SZ_MAX);

	// Damage bonuses
	for (int32 i = 0; i < RC_MAX; i++)
		vals[i] = sd->right_weapon.addrace[i];
	show_list("Physical Damage vs Race", race_names, vals, RC_MAX);
	for (int32 i = 0; i < SZ_MAX; i++)
		vals[i] = sd->right_weapon.addsize[i];
	show_list("Physical Damage vs Size", size_names, vals, SZ_MAX);
	for (int32 i = 0; i < ELE_MAX; i++)
		vals[i] = sd->right_weapon.addele[i];
	show_list("Physical Damage vs Element", ele_names, vals, ELE_MAX);
	for (int32 i = 0; i < RC_MAX; i++)
		vals[i] = sd->indexed_bonus.magic_addrace[i];
	show_list("Magic Damage vs Race", race_names, vals, RC_MAX);
	for (int32 i = 0; i < SZ_MAX; i++)
		vals[i] = sd->indexed_bonus.magic_addsize[i];
	show_list("Magic Damage vs Size", size_names, vals, SZ_MAX);
	for (int32 i = 0; i < ELE_MAX; i++)
		vals[i] = sd->indexed_bonus.magic_addele[i];
	show_list("Magic Damage vs Element", ele_names, vals, ELE_MAX);

	return 0;
}

/*==========================================
 * @spb - Show Party Buffs [Custom]
 *------------------------------------------*/
// Forward declaration for the timer
TIMER_FUNC(spb_timer);

// Buffs shown next to the names, in priority order (the first one is the most important). One character each.
// The player can hide any of them with "@spb hide <tag or name>" (the choice is kept in the character variable SPB_HIDE).
struct spb_buff_t { char tag; const char *name; int32 count; sc_type sc[4]; };
static const spb_buff_t spb_buffs[] = {
	{ '+', "Devotion",               1, { SC_DEVOTION } },
	{ 'F', "Chemical Protection",    4, { SC_CP_WEAPON, SC_CP_SHIELD, SC_CP_ARMOR, SC_CP_HELM } },   // any of the four
	{ 'S', "Striking",               1, { SC_STRIKING } },
	{ 'E', "Spell Enchanting",       1, { SC_SPELL_ENCHANTING } },
	{ 'X', "Expiatio",               1, { SC_EXPIATIO } },
	{ '!', "Full Shadow Protection", 1, { SC_PROTECTSHADOWEQUIP } },
	{ 'H', "Mediale Votum",          1, { SC_MEDIALE } },
	{ '1', "Presens Acies",          1, { SC_PRE_ACIES } },
	{ '2', "Argutus Telum",          1, { SC_A_TELUM } },
	{ '3', "Argutus Vita",           1, { SC_A_VITA } },
	{ 'A', "Increase AGI",           1, { SC_INCREASEAGI } },
	{ 'B', "Blessing",               1, { SC_BLESSING } },
};
static const int32 SPB_BUFFS = ARRAYLENGTH(spb_buffs);

static bool spb_has(map_session_data *msd, const spb_buff_t &b) {
	for (int32 k = 0; k < b.count; k++)
		if (msd->sc.getSCE(b.sc[k]))
			return true;
	return false;
}

// Writes "Name[+FAB]" into out (NAME_LENGTH bytes). Only the buffs this viewer did not hide are listed. The name is cut when it does not
// leave room for the tags; the closing bracket is never lost.
static void spb_decorate(const char *name, map_session_data *msd, uint32 hidden, char *out) {
	const int32 room = NAME_LENGTH - 1;                 // characters the party window can show for a name
	char tags[SPB_BUFFS + 1];
	int32 n = 0;
	if (msd) {
		for (int32 i = 0; i < SPB_BUFFS; i++)
			if (!(hidden & (1u << i)) && spb_has(msd, spb_buffs[i]))
				tags[n++] = spb_buffs[i].tag;
	}
	tags[n] = '\0';
	if (n == 0) { safestrncpy(out, name, NAME_LENGTH); return; }
	int32 shown = (int32)strlen(name);
	if (shown + 2 + n > room)
		shown = room - 2 - n;                           // at most 12 tags, so the name always keeps 9 letters
	snprintf(out, NAME_LENGTH, "%.*s[%s]", (int)shown, name, tags);
}

// Timer: builds the party list with the buff tags and sends it to the player, but only when it is different from what they have.
// (It used to be sent every 2 seconds whatever happened, which made the client's party window redraw for nothing.)
TIMER_FUNC(spb_timer) {
	map_session_data *sd = map_id2sd(id);
	if (!sd || sd->spb_tid == INVALID_TIMER || !sd->status.party_id) {
		if (sd) sd->spb_tid = INVALID_TIMER;
		return 0;
	}

	struct party_data *p = party_search(sd->status.party_id);
	if (!p) {
		sd->spb_tid = INVALID_TIMER;
		return 0;
	}

	PACKET_ZC_GROUP_LIST* pkt = reinterpret_cast<PACKET_ZC_GROUP_LIST*>(packet_buffer);
	pkt->packetType = partyinfo;
	pkt->packetLen = sizeof(*pkt);
	safestrncpy(pkt->partyName, p->party.name, sizeof(pkt->partyName));

	uint32 hash = 2166136261u;                          // FNV-1a over everything the list shows
	auto mix = [&hash](const void *data, size_t len) {
		const uint8 *b = static_cast<const uint8 *>(data);
		for (size_t k = 0; k < len; k++) { hash ^= b[k]; hash *= 16777619u; }
	};
	...
	for (int32 i = 0, c = 0; i < MAX_PARTY; i++) {
		...
		PACKET_ZC_GROUP_LIST_SUB &member = pkt->members[c];
		member.AID = m.account_id;
#if PACKETVER >= 20171207
		member.GID = m.char_id;
#endif
		map_session_data *msd = p->data[i].sd;
		if (msd && msd->m == sd->m)                     // buffs are only known for members on the same map
			spb_decorate(m.name, msd, sd->spb_hide, member.playerName);
		else
...
#endif
		mix(member.playerName, strlen(member.playerName));
		mix(member.mapName, strlen(member.mapName));
		mix(&member.leader, sizeof(member.leader));
		mix(&member.offline, sizeof(member.offline));
		mix(&m.class_, sizeof(m.class_));
		mix(&m.lv, sizeof(m.lv));
		mix(&sd->spb_hide, sizeof(sd->spb_hide));

		pkt->packetLen += static_cast<decltype(pkt->packetLen)>(sizeof(member));
		c++;
	}

	if (hash == 0) hash = 1;
	if (hash != sd->spb_hash) {                         // something changed (or the client was sent the plain list): send only to this player
		sd->spb_hash = hash;
		clif_send(pkt, pkt->packetLen, sd, SELF);
	}

	sd->spb_tid = add_timer(gettick() + 1000, spb_timer, sd->id, 0);
	return 0;
}

// which buff a word means: its tag ("+", "F", "a"...), or (part of) its name ("dev", "chemical", "vita")
static int32 spb_find(const char *word) {
	if (word[0] != '\0' && word[1] == '\0') {
		for (int32 i = 0; i < SPB_BUFFS; i++)
			if (toupper((unsigned char)word[0]) == toupper((unsigned char)spb_buffs[i].tag))
				return i;
		return -1;
	}
	size_t len = strlen(word);
	for (int32 i = 0; i < SPB_BUFFS; i++) {
		for (const char *w = spb_buffs[i].name; *w; w++)   // the start of the name or of any word in it
			if ((w == spb_buffs[i].name || w[-1] == ' ') && strnicmp(w, word, len) == 0)
				return i;
	}
	return -1;
}

static uint32 spb_load_hidden(map_session_data *sd) {
	return sd->spb_hide = (uint32)pc_readglobalreg(sd, reference_uid(add_str("SPB_HIDE"), 0));
}

static void spb_save_hidden(map_session_data *sd, uint32 mask) {
	sd->spb_hide = mask;
	sd->spb_hash = 0;                                    // send the party window again with the new choice
	pc_setglobalreg(sd, reference_uid(add_str("SPB_HIDE"), 0), (int64)mask);
}

// "[SPB] Showing: + F A B" / "[SPB] Hidden: X 1"
static void spb_say_state(const int32 fd, uint32 hidden) {
	char shown[64] = "", hid[64] = "", line[160];
	for (int32 i = 0; i < SPB_BUFFS; i++) {
		char t[4] = { ' ', spb_buffs[i].tag, '\0' };
		strcat((hidden & (1u << i)) ? hid : shown, t);
	}
	snprintf(line, sizeof(line), "[SPB] Showing:%s", shown[0] ? shown : " nothing");
	clif_displaymessage(fd, line);
	if (hid[0]) {
		snprintf(line, sizeof(line), "[SPB] Hidden:%s", hid);
		clif_displaymessage(fd, line);
	}
}

bool atcommand_spb_enabled(map_session_data *sd) {
	return sd != nullptr && sd->spb_tid != INVALID_TIMER;
}

// Turns the party buff bar on or off; returns false when it cannot be turned on (no party)
bool atcommand_spb_set(map_session_data *sd, bool on) {
	nullpo_retr(false, sd);
	if (on == atcommand_spb_enabled(sd))
		return true;
	if (on) {
		if (!sd->status.party_id)
			return false;
		spb_load_hidden(sd);
		sd->spb_hash = 0;
		sd->spb_tid = add_timer(gettick() + 500, spb_timer, sd->id, 0);
	} else {
		delete_timer(sd->spb_tid, spb_timer);
		sd->spb_tid = INVALID_TIMER;
		struct party_data *p = party_search(sd->status.party_id);
		if (p)
			clif_party_info(*p, sd);                     // plain names again
	}
	return true;
}

uint32 atcommand_spb_hidden(map_session_data *sd) {
	nullpo_retr(0, sd);
	return spb_load_hidden(sd);
}

void atcommand_spb_set_hidden(map_session_data *sd, uint32 mask) {
	nullpo_retv(sd);
	spb_save_hidden(sd, mask & ((1u << SPB_BUFFS) - 1));
}

/// @lobby: back to the lobby (hero_ent3 69,136) [Custom]
ACMD_FUNC(lobby)
{
	nullpo_retr(-1, sd);

	if (pc_isdead(sd)) {
		clif_displaymessage(fd, "You cannot use @lobby while dead.");
		return -1;
	}
	// GvG areas and castles while a siege is on: no way out with @lobby
	if (map_flag_gvg2(sd->m) || map_getmapflag(sd->m, MF_GVG) || map_getmapflag(sd->m, MF_GVG_DUNGEON) || map_getmapflag(sd->m, MF_GVG_TE)) {
		clif_displaymessage(fd, "You cannot use @lobby in a GvG area.");
		return -1;
	}
	if (!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE) &&
		(map_getmapflag(sd->m, MF_NOGO) || map_getmapflag(sd->m, MF_NOWARP) || map_getmapflag(sd->m, MF_NOTELEPORT) || sd->m == map_mapname2mapid("sp_jail"))) {
		clif_displaymessage(fd, "You cannot use @lobby on this map.");
		return -1;
	}
	uint16 mapidx = mapindex_name2id("hero_ent3");
	if (mapidx == 0 || pc_setpos(sd, mapidx, 69, 136, CLR_TELEPORT) != SETPOS_OK) {
		clif_displaymessage(fd, "The lobby is not available right now.");
		return -1;
	}
	clif_displaymessage(fd, "Welcome back to the lobby.");
	return 0;
}
...
ACMD_FUNC(spb)
{
	...
	char sub[32] = "", rest[256] = "";
	if (message && sscanf(message, "%31s %255[^\n]", sub, rest) < 1)
		sub[0] = '\0';

	// @spb list | show | hide | only | all : which buffs this player wants to see
	if (sub[0]) {
		uint32 hidden = spb_load_hidden(sd);
		if (!strcasecmp(sub, "list")) {
			clif_displaymessage(fd, "[SPB] Tag  Buff   (in this order of priority)");
			for (int32 i = 0; i < SPB_BUFFS; i++) {
				char line[120];
				snprintf(line, sizeof(line), "[SPB]  %c    %s  -  %s", spb_buffs[i].tag, spb_buffs[i].name, (hidden & (1u << i)) ? "hidden" : "shown");
				clif_displaymessage(fd, line);
			}
			clif_displaymessage(fd, "[SPB] Use: @spb hide <tag or name...> | @spb show <...> | @spb only <...> | @spb all");
			return 0;
		}
		if (!strcasecmp(sub, "all") || !strcasecmp(sub, "reset")) {
			spb_save_hidden(sd, 0);
			spb_say_state(fd, 0);
			return 0;
		}
		bool hide = !strcasecmp(sub, "hide"), show = !strcasecmp(sub, "show"), only = !strcasecmp(sub, "only");
		if (hide || show || only) {
			if (!rest[0]) {
				clif_displaymessage(fd, "[SPB] Which buffs? Tags or names, for example: @spb hide + F   or   @spb only A B X   (@spb list shows them all).");
				return -1;
			}
			uint32 mask = only ? ((1u << SPB_BUFFS) - 1) : hidden;    // "only" starts with everything hidden
			for (char *tok = strtok(rest, " ,"); tok; tok = strtok(nullptr, " ,")) {
				int32 i = spb_find(tok);
				if (i < 0) {
					char line[120];
					snprintf(line, sizeof(line), "[SPB] I do not know the buff '%s'. @spb list shows the tags and names.", tok);
					clif_displaymessage(fd, line);
					return -1;
				}
				if (hide) mask |= (1u << i); else mask &= ~(1u << i);   // show and only both reveal what they name
			}
			spb_save_hidden(sd, mask);
			spb_say_state(fd, mask);
			return 0;
		}
		clif_displaymessage(fd, "[SPB] Unknown option. Use @spb (on/off), @spb list, @spb hide/show/only <buffs>, @spb all.");
		return -1;
	}

	if (!sd->status.party_id) {
		clif_displaymessage(fd, "You are not in a party.");
		return -1;
	}

	if (sd->spb_tid != INVALID_TIMER) {
		// Disable - stop timer and send clean party info
		delete_timer(sd->spb_tid, spb_timer);
		sd->spb_tid = INVALID_TIMER;

		// Resend clean party info to restore original names
		struct party_data *p = party_search(sd->status.party_id);
		if (p)
			clif_party_info(*p, sd);

		clif_displaymessage(fd, "[SPB] Party buff display disabled.");
	} else {
		// Enable - start timer
		spb_load_hidden(sd);
		sd->spb_hash = 0;
		sd->spb_tid = add_timer(gettick() + 500, spb_timer, sd->id, 0);
		clif_displaymessage(fd, "[SPB] Party buff display enabled. Buffs will show in party window. (@spb list to choose what to see)");
	}
...
/*==========================================
 * @hold - Lock position, prevent movement on click [Custom]
 *------------------------------------------*/
ACMD_FUNC(hold)
{
	nullpo_retr(-1, sd);

	sd->hold_position = !sd->hold_position;

	if (sd->hold_position) {
		clif_displaymessage(fd, "[Hold] Position locked. You will not move when clicking.");
		// Stop current movement
		unit_stop_walking(sd, 1);
	} else {
		clif_displaymessage(fd, "[Hold] Position unlocked. Normal movement restored.");
	}
