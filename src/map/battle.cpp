...
int32 battle_check_target( block_list *src, block_list *target,int32 flag)
{
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_GUILD|BCT_ENEMY) )
		{
			...
			if( !((mapdata->getMapFlag(MF_PVP) || mapdata_flag_gvg(mapdata)) && mapdata->getMapFlag(MF_PVP_NOGUILD)) && s_guild && t_guild && (s_guild == t_guild || (!(flag&BCT_SAMEGUILD) && guild_isallied(s_guild, t_guild))) && (!mapdata->getMapFlag(MF_BATTLEGROUND) || sbg_id == tbg_id) )
				state |= BCT_GUILD;
			else
				state |= BCT_ENEMY;
		}
