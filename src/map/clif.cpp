...
// === @hide summons [Custom] ===
void clif_getareachar_unit( map_session_data* sd, block_list *bl );   // defined further down
// Summons this player chose not to see. A unit is judged by what the server knows about its id (map_id2bl), so every packet that names it
// (entry, move, stop, vanish, attacks, skills, effects, status, names...) is dropped, and the client never hears of it. Wargs are not units:
// the flag is taken out of the copy of the packet this player gets.
static bool clif_summon_hidden(map_session_data *sd, uint32 mask, uint32 id)
{
	block_list *b = map_id2bl(id);
	if (b == nullptr || b == sd)
		return false;
	switch (b->type) {
	case BL_PET: {
		pet_data *pd = reinterpret_cast<pet_data *>(b);
		return (pd->master == sd) ? (mask & SH_PET_OWN) != 0 : (mask & SH_PET_OTHERS) != 0;
	}
	case BL_ELEM:
		return (mask & SH_ELEMENTAL) && reinterpret_cast<s_elemental_data *>(b)->master != sd;
	case BL_HOM:
		return (mask & SH_HOMUNCULUS) && reinterpret_cast<homun_data *>(b)->master != sd;
	case BL_MOB: {
		mob_data *md = reinterpret_cast<mob_data *>(b);
		if (md->special_state.ai == AI_ABR)
			return (mask & SH_ABR) && md->master_id != sd->id;
		if (md->special_state.ai == AI_BIONIC)
			return (mask & SH_BIONIC) && md->master_id != sd->id;
		return false;
	}
	}
	return false;
...
// 0 = send as it is, 1 = drop, 2 = send the copy in tmp (tmp holds len bytes)
// Packet ids and field offsets are for PACKETVER 20250604.
static int32 clif_summonhide_filter(map_session_data *sd, const void *buf, int32 len, unsigned char *tmp)
{
	const uint32 m = sd->summon_hide;
	if (!m || len < 6 || len > 1024)
		return 0;

	static const struct { uint16 id; int8 a, b; } ids[] = {
		{ 0x9fd, 5, -1 }, { 0x9fe, 5, -1 }, { 0x9ff, 5, -1 },        // unit entries (walking / new / idle): id at 5
		{ 0x86, 2, -1 }, { 0x88, 2, -1 }, { 0x80, 2, -1 },           // move, stop, vanish
		{ 0x8c8, 2, 6 }, { 0x2e1, 2, 6 }, { 0x8a, 2, 6 },            // attacks: source, target
		{ 0x1de, 4, 8 }, { 0x114, 4, 8 }, { 0x115, 4, 8 },           // skills with target: caster, target
		{ 0x9cb, 8, 12 }, { 0x11a, 6, 10 }, { 0x117, 4, -1 },        // skills without damage, ground skill cast
		{ 0xc0, 2, -1 }, { 0x9c, 2, -1 }, { 0x1aa, 2, -1 },          // emotion, direction, pet action
		{ 0x1f3, 2, -1 }, { 0x284, 2, -1 }, { 0xb69, 2, -1 }, { 0x19b, 2, -1 }, { 0x1d0, 2, -1 },   // effects
		{ 0x983, 4, -1 }, { 0x196, 4, -1 }, { 0x43f, 4, -1 },        // status effects
		{ 0x95, 2, -1 }, { 0x195, 2, -1 }, { 0xa30, 2, -1 },         // names
	};
	const uint16 id = RBUFW(buf, 0);
	for (const auto &e : ids) {
		if (e.id != id)
			continue;
		if (clif_summon_hidden(sd, m, RBUFL(buf, e.a)))
			return 1;
		if (e.b >= 0 && len >= e.b + 4 && clif_summon_hidden(sd, m, RBUFL(buf, e.b)))
			return 1;
		break;
	}

	if (m & SH_WARG) {
		// entries of players carry their option flags at 19 (objecttype at 4); a state change has them at 10
		int32 optoff = -1;
		if ((id == 0x9fd || id == 0x9fe || id == 0x9ff) && RBUFB(buf, 4) == 0 && len >= 23)
			optoff = 19;
		else if (id == 0x229 && len >= 15)
			optoff = 10;
		if (optoff >= 0) {
			const uint32 who = RBUFL(buf, id == 0x229 ? 2 : 5);
			block_list *b = map_id2bl(who);
			if (b != nullptr && b->type == BL_PC && b != sd && (RBUFL(buf, optoff) & OPTION_WUG)) {
				memcpy(tmp, buf, len);
				WBUFL(tmp, optoff) = RBUFL(buf, optoff) & ~static_cast<uint32>(OPTION_WUG);
				return 2;
			}
		}
	}
...
static int32 clif_summonhide_refresh_sub(block_list *bl, va_list ap)
{
	map_session_data *sd = va_arg(ap, map_session_data *);
	uint32 old_mask = va_arg(ap, uint32);
	if (bl == sd)
		return 0;
	if (bl->type == BL_PC) {                                       // wargs: send the option state again, it goes through the filter with the new mask
		if ((old_mask ^ sd->summon_hide) & SH_WARG) {
			status_change *sc = status_get_sc(bl);
			if (sc != nullptr && (sc->option & OPTION_WUG))
				clif_changeoption_target(bl, sd);
		}
		return 0;
	}
	bool was = clif_summon_hidden(sd, old_mask, bl->id), now = clif_summon_hidden(sd, sd->summon_hide, bl->id);
	// what was hidden and is not any more has to be shown; what is hidden now has to vanish
	if (now && !was) {
		PACKET_ZC_NOTIFY_VANISH packet{};
		packet.packetType = HEADER_ZC_NOTIFY_VANISH;
		packet.gid = bl->id;
		packet.type = static_cast<decltype(packet.type)>(CLR_OUTSIGHT);
		WFIFOHEAD(sd->fd, sizeof(packet));
		memcpy(WFIFOP(sd->fd, 0), &packet, sizeof(packet));         // straight to the client: clif_send would filter it
...
void clif_summonhide_refresh(map_session_data *sd, uint32 old_mask)
{
	nullpo_retv(sd);
	if (!session_isActive(sd->fd))
		return;
	map_foreachinallarea(clif_summonhide_refresh_sub, sd->m, sd->x - AREA_SIZE, sd->y - AREA_SIZE, sd->x + AREA_SIZE, sd->y + AREA_SIZE,
		BL_PC | BL_PET | BL_HOM | BL_ELEM | BL_MOB, sd, old_mask);
}
// === End @hide summons [Custom] ===

// === @packetfilter [Custom] ===
// Decides whether a visual packet about OTHER players is dropped for this receiver. Only decoration is touched, and anything the receiver
// takes part in is never dropped: their own attacks, hits on them, skills aimed at them, their own skill units.
// Field offsets are for PACKETVER 20250604 (ZC_NOTIFY_ACT 0x08c8, ZC_NOTIFY_SKILL 0x01de, ZC_USE_SKILL 0x09cb, skill unit 0x09ca).
static bool clif_packetfilter_drops(map_session_data *sd, block_list *src_bl, const void *buf, int32 len)
{
	const uint32 f = sd->packet_filter;
	if (!f || len < 2)
		return false;

	const uint16 id = RBUFW(buf, 0);
	const bool own = (src_bl == sd);                     // the packet is about the receiver himself
	auto at = [&](int32 off) -> uint32 { return len >= off + 4 ? RBUFL(buf, off) : 0; };
	auto mine = [&](int32 off) -> bool { return at(off) == (uint32)sd->id; };

	switch (id) {
	case 0x1c8:                                            // ZC_USE_ITEM_ACK
		return (f & PF_ITEM) && !own;
	case 0xc0:                                             // ZC_EMOTION
		return (f & PF_EMOTE) && !own;
	case 0x8d:                                             // ZC_NOTIFY_CHAT (public chat of someone else)
		return (f & PF_CHAT) && !own;
	case 0x196: case 0x43f: case 0x983:                    // ZC_MSG_STATE_CHANGE (status icon/effect of the subject)
		return (f & PF_BUFF_ANIM) && !own;
	case 0x8c8: case 0x2e1: case 0x8a:                     // ZC_NOTIFY_ACT: srcID at 2, targetID at 6
		return (f & PF_ATK_ANIM) && !mine(2) && !mine(6);
	case 0x1de: case 0x114: case 0x115:                    // ZC_NOTIFY_SKILL: AID at 4, targetID at 8
		return (f & PF_SKILL_TARGET) && !mine(4) && !mine(8);
	case 0x9cb:                                            // ZC_USE_SKILL: targetAID at 8, srcAID at 12
		return (f & PF_SKILL_TARGET) && !mine(8) && !mine(12);
	case 0x11a:                                            // ZC_USE_SKILL (old layout): targetAID at 6, srcAID at 10
		return (f & PF_SKILL_TARGET) && !mine(6) && !mine(10);
	case 0x117:                                            // ZC_NOTIFY_GROUNDSKILL: AID at 4
		return (f & PF_SKILL_GROUND) && !mine(4);
	case 0x9ca:                                            // ZC_SKILL_ENTRY5: a skill unit appears (creator at 8, unit id at 16)
		if (mine(8))
			return false;                                  // your own units are always shown
		if (f & PF_SKILL_UNITS)
			return true;
		if (f & PF_SONG_DANCE) {
			switch (at(16)) {
			case UNT_LULLABY: case UNT_RICHMANKIM: case UNT_ETERNALCHAOS: case UNT_DRUMBATTLEFIELD: case UNT_RINGNIBELUNGEN:
			case UNT_ROKISWEIL: case UNT_INTOABYSS: case UNT_SIEGFRIED: case UNT_DISSONANCE: case UNT_WHISTLE:
			case UNT_ASSASSINCROSS: case UNT_POEMBRAGI: case UNT_APPLEIDUN: case UNT_UGLYDANCE: case UNT_HUMMING:
			case UNT_DONTFORGETME: case UNT_FORTUNEKISS: case UNT_SERVICEFORYOU:
				return true;
			}
		}
		return false;
	case 0x9c:                                             // ZC_CHANGE_DIRECTION
		return (f & PF_DIR_CHANGE) && !own;
	case 0x1aa:                                            // ZC_PET_ACT
		return (f & PF_PET_EMOTION) && !own;
	case 0x1f3: case 0x284: case 0xb69: case 0x19b:        // ZC_NOTIFY_EFFECT*: GID at 2
	case 0x1d0:                                            // ZC_SPIRITS
		return (f & PF_AURA) && !own && !mine(2);
	case 0xa3b:                                            // ZC_HAT_EFFECT
		return (f & PF_HAT_EFFECT) && !own;
	}
	return false;
}
// === End @packetfilter [Custom] ===
...
static int32 clif_send_sub(block_list *bl, va_list ap)
{
	...
	if (clif_packetfilter_drops(sd, src_bl, buf, len))
		return 0;

	unsigned char shtmp[1024];                          // @hide summons: the packet may be replaced by a changed copy
	const void *sendbuf = buf;
	if (sd->summon_hide) {
		int32 r = clif_summonhide_filter(sd, buf, len, shtmp);
		if (r == 1)
			return 0;
		if (r == 2)
			sendbuf = shtmp;
	...
	memcpy(WFIFOP(fd,0), sendbuf, len);
	WFIFOSET(fd,len);
...
int32 clif_send(const void* buf, int32 len, block_list* bl, enum send_target type)
{
	...
	case SELF:
		if( clif_session_isValid(sd) ){
			if (clif_packetfilter_drops(sd, bl, buf, len))
				break;
			unsigned char shtmp[1024];
			const void *sendbuf = buf;
			if (sd->summon_hide) {
				int32 r = clif_summonhide_filter(sd, buf, len, shtmp);
				if (r == 1)
...
void clif_party_info( struct party_data& party, map_session_data* sd ){
	...
	// @spb: whoever receives the plain list loses the buff tags, so their timer has to send the tagged one again
	if( sd != nullptr )
		sd->spb_hash = 0;
	else
		for( int32 i = 0; i < MAX_PARTY; i++ )
			if( party.data[i].sd != nullptr )
				party.data[i].sd->spb_hash = 0;
...
void clif_parse_Restart(int32 fd, map_session_data *sd)
{
	...
	case 0x00:
		// playground: nobody leaves a 2026arena map by respawning; the dead wait for the end of the round (the arena script revives them)
		if (sd->m >= 0 && map_getmapdata(sd->m) != nullptr && strncmp(map_getmapdata(sd->m)->name, "2026arena", 9) == 0) {
			clif_displaymessage(sd->fd, "You can not return to the lobby during an arena match. Wait for the end of the round.");
			break;
		}
...
void clif_parse_roulette_open( int32 fd, map_session_data* sd ){
	...
	// [Custom] the roulette button opens the player options menu (the NPC PlayerOptionsMenu, npc/custom/player_options_menu.txt) instead of the roulette.
	// Without that NPC loaded, or while the player is already talking to an NPC, the roulette behaves as before.
	if (sd->npc_id == 0 && npc_event_do_id("PlayerOptionsMenu::OnOpen", sd->id) > 0)
		return;
