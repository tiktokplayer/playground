 ...
 *------------------------------------------*/
/// Roulette menu, hide summons: sets the mask, keeps it in HIDE_SUM and makes what is in view follow the change
void pc_summonhide_set(map_session_data *sd, uint32 mask)
{
	nullpo_retv(sd);
	uint32 old = sd->summon_hide;
	sd->summon_hide = mask & SH_ALL;
	pc_setglobalreg(sd, add_str("HIDE_SUM"), static_cast<int64>(sd->summon_hide));
	if (sd->state.active && old != sd->summon_hide)
		clif_summonhide_refresh(sd, old);
}

/// @packetfilter: sets the filter and keeps it in the character variable PF_MASK so it is there at the next login
void pc_packetfilter_set(map_session_data *sd, uint32 mask)
{
	nullpo_retv(sd);
	sd->packet_filter = mask & PF_ALL;
	pc_setglobalreg(sd, add_str("PF_MASK"), static_cast<int64>(sd->packet_filter));
}
...
void pc_reg_received(map_session_data *sd)
{
	...
	sd->summon_hide = static_cast<uint32>(pc_readglobalreg(sd, add_str("HIDE_SUM"))) & SH_ALL;
	sd->packet_filter = static_cast<uint32>(pc_readglobalreg(sd, add_str("PF_MASK"))) & PF_ALL; // @packetfilter choice of the last session
...
int32 pc_dead(map_session_data *sd,block_list *src)
{
	...
	if (src && src->type == BL_PC) {
		...
		npc_script_event( *ssd, NPCE_KILLPC );
				// === @battleinfo notification [Custom] ===
		{
			uint16 kill_skill = sd->last_dmg_skill;
			// a hit that was bounced back onto the victim (Reflect Shield, Reflect Damage, Kaite, Magic Mirror...) is shown as "Reflect Damage"
			bool reflected = kill_skill == CR_REFLECTSHIELD || kill_skill == LG_REFLECTDAMAGE ||
				(kill_skill > 0 && kill_skill == sd->last_reflect_skill && gettick() - sd->last_reflect_tick <= 2000);
			const char *skill_desc = reflected ? "Reflect Damage" : (kill_skill > 0) ? skill_get_desc(kill_skill) : "Normal Attack";

			// Notify killer: in the guild-chat colour, "You killed <name> - <job> using [<skill>]"
			if (ssd->battleinfo) {
				char msg[256];
				snprintf(msg, sizeof(msg), "You killed %s - %s using [%s]", sd->status.name, job_name(sd->status.class_), skill_desc);
				clif_disp_message(ssd, msg, strlen(msg), SELF);
			}
			// Notify victim: same style, "You were killed by <name> - <job> using [<skill>]"
			if (sd->battleinfo) {
				char msg[256];
				snprintf(msg, sizeof(msg), "You were killed by %s - %s using [%s]", ssd->status.name, job_name(ssd->status.class_), skill_desc);
				clif_disp_message(sd, msg, strlen(msg), SELF);
			}
		}
		// === End @battleinfo [Custom] ===
	...
	//GvG
	if( mapdata_flag_gvg2(mapdata) ) {
		// playground: the 7 vs 7 arenas do not send the dead back to their save point; the arena script revives them at the end of the round
		if( strncmp(mapdata->name, "2026arena", 9) != 0 )
			sd->respawn_tid = add_timer(tick+1000, pc_respawn_timer, sd->id, 0);
		return 1|8;
	}
	else if( sd->bg_id ) {
