...
/// Returns the @packetfilter mask of the attached player (see enum e_packet_filter in pc.hpp) [Custom]
///
/// getpacketfilter() -> <int>
BUILDIN_FUNC(getpacketfilter)
{
	map_session_data *sd;
	...
	script_pushint(st, sd->packet_filter);
	return SCRIPT_CMD_SUCCESS;
}

/// Sets the @packetfilter mask of the attached player; it is kept for the next login [Custom]
///
/// setpacketfilter <mask>;
BUILDIN_FUNC(setpacketfilter)
{
	map_session_data *sd;

	if (!script_rid2sd(sd))
		return SCRIPT_CMD_FAILURE;
	pc_packetfilter_set(sd, static_cast<uint32>(script_getnum(st, 2)));
	return SCRIPT_CMD_SUCCESS;
}

/// Party buff bar (@spb) of the attached player, for the roulette menu [Custom]
///
/// getspb() -> <int>          1 when the bar is on
/// setspb(<0|1>) -> <int>     turns it on/off; returns 0 when it cannot be turned on (not in a party)
/// getspbhide() -> <int>      bit i set = buff i of the table is hidden
/// setspbhide <mask>;
BUILDIN_FUNC(getspb)
{
	map_session_data *sd;
	...
	script_pushint(st, atcommand_spb_enabled(sd) ? 1 : 0);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(setspb)
{
	...
	script_pushint(st, atcommand_spb_set(sd, script_getnum(st, 2) != 0) ? 1 : 0);   // 0 = could not (not in a party)
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(getspbhide)
{
	map_session_data *sd;
	...
	script_pushint(st, atcommand_spb_hidden(sd));
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(setspbhide)
{
	map_session_data *sd;

	if (!script_rid2sd(sd))
		return SCRIPT_CMD_FAILURE;
	atcommand_spb_set_hidden(sd, static_cast<uint32>(script_getnum(st, 2)));
	return SCRIPT_CMD_SUCCESS;
}

/// Summons the attached player does not want to see (bits of enum e_summon_hide in pc.hpp) [Custom]
///
/// getsummonhide() -> <int>
/// setsummonhide <mask>;
BUILDIN_FUNC(getsummonhide)
{
	map_session_data *sd;
	...
	script_pushint(st, sd->summon_hide);
	return SCRIPT_CMD_SUCCESS;
}

BUILDIN_FUNC(setsummonhide)
{
	map_session_data *sd;

	if (!script_rid2sd(sd))
		return SCRIPT_CMD_FAILURE;
	pc_summonhide_set(sd, static_cast<uint32>(script_getnum(st, 2)));
	return SCRIPT_CMD_SUCCESS;
}
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(getpacketfilter,""),
	BUILDIN_DEF(setpacketfilter,"i"),
	BUILDIN_DEF(getspb,""),
	BUILDIN_DEF(setspb,"i"),
	BUILDIN_DEF(getspbhide,""),
	BUILDIN_DEF(setspbhide,"i"),
	BUILDIN_DEF(getsummonhide,""),
	BUILDIN_DEF(setsummonhide,"i"),
	BUILDIN_DEF(end,""),
